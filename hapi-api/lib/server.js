...
exports.start = async () => {
  // [Start server for general use]
    ...
    let swaggered = false;
    if (process.env.HAPI_PORT) {
      console.log('When using docker-compose');
      // server.table().forEach((route) => console.log(`${route.method}\thttp://${process.env.HAPI_HOST}:${process.env.HAPI_PORT}${route.path}`));
      server.table().forEach((route) => {
        if (!route.path.includes('swagger')) {
          console.log(`${route.method}\thttp://${process.env.HAPI_HOST}:${process.env.HAPI_PORT}${route.path}`);
        } else {
          swaggered = true;
        }
      });
